...
class OportunidadController extends Controller
{
    ...
    public function index(Request $request)
    {
        $query = $this->applyArchivedFilter(
            CrmAccess::applyClienteScope(
                Oportunidad::query()->with('cliente')->withCount(['actividades', 'tareas']),
                $request->user()
            ),
            $request
        );
    ...
    {
        CrmAccess::ensureCanAccessScopedCliente(request()->user(), $oportunidad->cliente_id);

        $oportunidad->loadCount(['actividades', 'tareas'])
            ->loadMissing([
                'cliente',
